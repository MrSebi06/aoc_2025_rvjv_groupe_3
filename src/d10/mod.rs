...
pub fn p1(input: &str) -> u64 {
    ...
    for line in input.lines()
    {
        ...
        for (i, button_str) in split.enumerate()
        {
            ...
            if button_str.starts_with('{') { continue }

            let mut triggers = vec![0; 0];

            let button_str = &button_str[1..button_str.len()-1];
            let button_triggers = button_str.split(",");
            for trig_str in button_triggers { triggers.push(trig_str.parse::<usize>().unwrap()) }
        ...
        while !queue.is_empty() {
            ...
            for i in button { lights_state[*i] = !lights_state[*i] }
