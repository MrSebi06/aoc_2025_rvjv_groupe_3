...
pub fn p1(input: &str) -> u64 {
    ...
    for segment in segments
    {
        ...
        let lower: u64 = bounds.next().expect("lower not found").to_string().parse().expect("lower is not u32");
        let upper: u64 = bounds.next().expect("upper not found").to_string().parse().expect("upper is not u32");
